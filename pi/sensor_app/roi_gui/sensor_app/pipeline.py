...
def run_once(node: Node, drivers, ocr, db, cloud=None) -> List[dict]:
    ...
    # --- ADD THIS CHECK ---
    if frame is None:
        logger.warning("Camera frame not ready yet for node %s, skipping pass...", node.node_id)
        return results
    # ----------------------

    box_dicts = [b.to_box_dict() for b in node.boxes]
    
    # Bypass feature alignment for performance
    warped_boxes = box_dicts
    for wb in warped_boxes:
        wb["in_bounds"] = True
...
def run_loop(nodes, drivers, ocr, db, cloud=None, cadence_s: float = 2.0, n_iterations: Optional[int] = None):
